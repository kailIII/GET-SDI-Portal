...
OpenLayers.Control.Trace = OpenLayers.Class(OpenLayers.Control, {
    ...
    activate: function() {
		maptab_edit_controls["drawpolygon"].handler.freehand=true;
    ...
    deactivate: function() {
		maptab_edit_controls["drawpolygon"].handler.freehand=false;
	...
	onSnap: function(event){
		...
		if (event.snappedFeature.geometry.CLASS_NAME == "OpenLayers.Geometry.Polygon")	{
			...
			var t=[];
			for(var k in this.snappedGeometry)
			{
				t.push(k+":"+this.snappedGeometry[k]);
			}
			
			//alert(t.join('\r\n'));
			
		}else if (event.snappedFeature.geometry.CLASS_NAME == "OpenLayers.Geometry.MultiPolygon")	{

			var points=event.snappedFeature.geometry.components[0].getVertices();
			...
			this.snappedGeometry =new OpenLayers.Geometry.LinearRing(_p);

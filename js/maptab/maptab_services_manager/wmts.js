					...
					handler:function()
					{
						...
						Ext.each(selected,function(item)
						{
							if (item.get("_layerObject")._loadedStatus>0)
							{
								...
								mapRemoveLayerNode(item.get("_layerObject")._layer);
...
function wmts_map_layer(_serviceObject,_layerObject)
{
	...
	var layer= new OpenLayers.Layer.WMTS({
		...
		url:fn_surl(_serviceObject._serviceUrl,_serviceObject._username,_serviceObject._password,'proxy'),
		layer: _layerObject._layerName,
		matrixSet: _layerObject._extras._matrix[mapGetCurrentProjection()]._matrixSet,
		matrixIds: _layerObject._extras._matrix[mapGetCurrentProjection()]._matrixIds,
		format: _layerObject._layerFormat,
		transparent: _layerObject._transparent,
		tileOrigin:new OpenLayers.LonLat(_layerObject._extras._matrix[mapGetCurrentProjection()]._tileOriginX,_layerObject._extras._matrix[mapGetCurrentProjection()]._tileOriginY),
		style: "",
		isBaseLayer: false,
		tileFullExtent:mapLayerBounds(_layerObject),
		projection:new OpenLayers.Projection(_layerObject._extras._matrix[mapGetCurrentProjection()]._projection),
